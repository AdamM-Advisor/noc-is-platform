  ...
  return (
    <div className="space-y-5">
      <Section title="Profil Tipikal">
        ...
        <div className="mt-2 text-xs text-[#64748B]">Sample: {(data.sample_size || 0).toLocaleString()} tiket</div>
      </Section>
      ...
      {coAlarms && coAlarms.length > 0 && (
        <Section title="Co-occurring Alarms">
          <div className="space-y-2">
            {coAlarms.map((ca, i) => (
              <div key={i} className="flex items-center gap-3 bg-white rounded-lg border border-gray-200 p-3 shadow-sm">
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-semibold text-[#0F172A] truncate">{ca.co_alarm_rc_1 || ca.co_alarm_description}</div>
                  <div className="text-xs text-[#64748B] mt-0.5">
                    {ca.co_alarm_rc_category} | {ca.typical_lag_description || `Lag: ${ca.typical_lag_min || 0}m`}
                  </div>
                </div>
                <div className="text-right shrink-0">
                  <div className="text-sm font-bold text-[#0F172A]">{(ca.co_occurrence_pct || 0).toFixed(1)}%</div>
                  <div className="text-xs text-[#64748B]">{ca.sample_size} tiket</div>
                </div>
...
function KV({ label, value }) {
  return (
    <div className="flex items-baseline gap-2 py-1.5">
      <span className="text-xs font-semibold text-[#475569] min-w-[100px] shrink-0">{label}</span>
