...
- **Data Processing**:
    - **Site Master**: Auto-populates hierarchy (Regional, NOP, TO) and enriches sites based on rules. Uses bulk DuckDB operations (DataFrame registration + INSERT...ON CONFLICT upsert) for fast processing — 78K rows in ~2 seconds. Vectorized enrichment via pandas instead of row-by-row. Progress callback support for UI feedback.
